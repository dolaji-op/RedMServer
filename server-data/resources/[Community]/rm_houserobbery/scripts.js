$(document).ready(function(){
    ...
    document.onkeydown = function (data) {
        ...
        if (data.which == 69) {
          closeMain()
          $.post('http://lto_houserobbery/taskEnd', JSON.stringify({taskResult: percent})); // change the name of the script here if you rename it
        }
    }
    ...
    window.addEventListener('message', function(event){
      ...
      if(item.closeFail === true) {
        closeMain()
        $.post('http://lto_houserobbery/taskCancel', JSON.stringify({tasknum: curTask})); // change the name of the script here if you rename it
      }

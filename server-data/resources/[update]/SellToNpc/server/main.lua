...
--------------------------------------CHECK PD----------------------------------------
    ...
    if checkPD == 0 then
        TriggerClientEvent("vorp:TipRight", _source, 'Not enough sheriffs in town, coward come back when there are no more people', 6000)
 ...
 --------------------------------------------------------------------------------  
        ...
        if sellFcked == 1 then
            if _x == 0 then
                TriggerClientEvent("vorp:TipRight", _source, 'You have nothing to sell', 6000)    
            
            
            else 
                print(json.encode(luck))
                if luck > 4 then
                    TriggerClientEvent("vorp:TipRight", _source, 'I dont want your stuff stupid', 6000)
                else
                    ...
                    if count >= AmountToSell then
                        ...
                        TriggerClientEvent("vorp:TipRight", _source, 'You have sold '..AmountToSell.. ' ' ..randomitem.. ' pour ' ..price..' dollars ', 6000)
                        local webhook = Config.WebHook
  	                    SendWebhook(webhook, _source, text)
                    else
                        TriggerClientEvent("vorp:TipRight", _source, 'I want '..AmountToSell..' '..randomitem..' and you dont have much!', 6000) 
                    end
                end
            end
        else
            TriggerClientEvent("vorp:TipRight", _source, 'You re not in the right city, go look elsewhere if Im there', 6000)
        end
    end
end)

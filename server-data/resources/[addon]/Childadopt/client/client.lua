			...
			for i = 1, #pets do
				...
				if acheck == true then
					if WarMenu.Button(pets[i]['Text'], pets[i]['SubText'], pets[i]['Desc']) then
						TriggerServerEvent('ac:buydog', pets[i]['Param'])
						WarMenu.CloseMenu()
					end
				end
			end
...
RegisterNetEvent( 'ac:removedog' )
AddEventHandler( 'ac:removedog', function (args)
...
RegisterNetEvent( 'ac:putaway' )
AddEventHandler( 'ac:putaway', function (args)
	if currentPetPed then
		DeleteEntity(currentPetPed)
...
RegisterCommand("gohomechild", function(source, args, rawCommand) --  COMMAND
	TriggerEvent('ac:putaway')
end)

RegisterCommand("callchild", function(source, args, rawCommand) --  COMMAND
	TriggerServerEvent('ac:loaddog')
end)
...
RegisterNetEvent( 'ac:spawndog' )
AddEventHandler( 'ac:spawndog', function ( dog, skin, isInShop )
